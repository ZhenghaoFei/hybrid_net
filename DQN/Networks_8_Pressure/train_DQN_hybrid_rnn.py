...
SUMMARY_DIR = './results_dqn_hybrid_rnn/'
RANDOM_SEED = 1234
...
class QNetwork(object):
    ...
    def create_Q_network(self):
        ...
        features = tflearn.conv_2d(inputs, 3, 1, activation='relu', name='conv1')
        features = tflearn.conv_2d(features, 5, 3, activation='relu', name='conv2')
        features = tflearn.layers.conv.max_pool_2d (features, 2, strides=None, padding='same', name='MaxPool2D1')

        features = tflearn.conv_2d(features, 3, 3, activation='relu', name='conv3')

        # rnn
        features_rnn = tflearn.layers.core.flatten(features)
        fc1 = tflearn.fully_connected(features_rnn, 16)
        fc2 = tflearn.fully_connected(fc1, 8)
        fc_fb = tflearn.fully_connected(fc2, 16)
        ...
        # plain
        net_plain = tflearn.fully_connected(features, 16, activation='relu')
        net_plain = tflearn.fully_connected(net_plain, 8, activation='relu')

        # combine
        alpha  = tflearn.fully_connected(features, 16, activation='relu')
        alpha  = tflearn.fully_connected(features, 1, activation='relu')
