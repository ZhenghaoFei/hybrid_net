...
PLAN_ITER_NUM = 8
SUMMARY_DIR = "./summary_hybird_k_dynamic"
...
def atari_model(img_in, num_actions, scope, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("plan_module"):
            ...
            #   net
            beta = tf.Variable(0.5*np.ones(PLAN_ITER_NUM), name='beta', dtype=tf.float32)
            plan_net = tf.nn.relu(tf.matmul(features,fc1_w) + fc1_b)
            plan_out = beta[0]*tf.nn.relu(tf.matmul(plan_net,fc2_w) + fc2_b)
            ...
            for i in range(PLAN_ITER_NUM - 1):
                ...
                plan_out +=  beta[i+1] * tf.nn.relu(tf.matmul(plan_net,fc2_w) + fc2_b)

            # plan_net = tf.nn.relu(tf.matmul(plan_net,fc2_w) + fc2_b)

        with tf.variable_scope("action_value"):
             # combine
            hidden_plan = layers.fully_connected(plan_out, 512, activation_fn=tf.nn.relu)
